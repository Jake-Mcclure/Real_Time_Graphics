...
void Model::traverseNode(unsigned int nextNode, glm::mat4 matrix)
{
	...
	rotat = glm::mat4_cast(rotation);
	scal = glm::scale(scal, scale);
	trans = glm::translate(trans, translation);
	glm::mat4 matNextNode = matrix * matNode * trans * rotat * scal;
...
std::vector<float> Model::getFloats(json accessor)
{
	...
	unsigned int amountPerVert;
	if (type == "SCALAR") amountPerVert = 1;
	else if (type == "VEC2") amountPerVert = 2;
	else if (type == "VEC3") amountPerVert = 3;
	else if (type == "VEC4") amountPerVert = 4;
	else throw std::invalid_argument("Type is invalid (not SCALAR, VEC2, VEC3, or VEC4)");

	unsigned int beginningOfData = byteOffset + accByteOffset;
	unsigned int lengthOfData = count * 4 * amountPerVert;
